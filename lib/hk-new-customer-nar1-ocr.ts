...
Director address mapping (Correspondence Address / 通訊地址):
...
- nameEn: combine English surname and other names (e.g. SAKON HIROKI).
- If a field is unreadable, use null. directors may be empty array if none found.
- Read all pages to find company details and all directors.
`.trim()

function normalizeDirector(raw: unknown): Nar1Director | null {
  if (!raw || typeof raw !== "object") return null
  const record = raw as Record<string, unknown>
  const nameEn = typeof record.nameEn === "string" ? record.nameEn.trim() : ""
