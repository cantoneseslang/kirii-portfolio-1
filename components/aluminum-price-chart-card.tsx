...
import type { DashboardAluminumPriceResponse } from "@/types/dashboard-aluminum-price"

type LoadState = "loading" | "success" | "error"
type AluminumPriceChartCardProps = {
  variant?: "preview" | "detail"
}

const DETAIL_PAGE_HREF = "/dashboard/aluminum-price"
const CHART_SPEC_LABEL = "长江/南海"
const FIXED_Y_AXIS_DOMAIN: [number, number] = [19000, 26000]
const FIXED_Y_AXIS_TICKS = [19000, 20000, 21000, 22000, 23000, 24000, 25000, 26000]
...
function fmtPctAl(current: number | null, base: number | null) {
...
function buildComparisonRows(
  points: DashboardAluminumPriceResponse["points"],
  series: DashboardAluminumPriceResponse["series"]
): CompRow[] {
...
export default function AluminumPriceChartCard({
  variant = "preview",
}: AluminumPriceChartCardProps) {
  const [liveData, setLiveData] = useState<DashboardAluminumPriceResponse | null>(null)
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        const response = await fetch(`/api/dashboard/aluminum-price?${params.toString()}`, {
        ...
        if (!response.ok) {
          ...
          throw new Error(payload?.error || "Failed to load aluminum price data")
        }

        const payload = (await response.json()) as DashboardAluminumPriceResponse
        setLiveData(payload)
        setLoadState("success")
      } catch (error) {
        if (controller.signal.aborted) return
        setLoadState("error")
        setErrorMessage(error instanceof Error ? error.message : "Failed to load aluminum price data")
      }
    }
    ...
    <div className="rounded-lg border bg-white p-1 sm:rounded-xl sm:p-3">
      ...
      >
        <LineChart
          ...
          margin={{ top: 8, right: 2, left: -4, bottom: isPreview ? 14 : 24 }}
        >
                    ...
                    <thead>
                      <tr className="border-b border-slate-200 text-muted-foreground">
                        ...
                        {liveData.series.map((s) => (
                          <th key={s.key} className="py-1 pr-1 text-right font-medium text-[9px] sm:text-[10px]" title={s.label}>
                            <span style={{ display: "inline-block", width: 8, height: 8, borderRadius: "50%", backgroundColor: s.color, marginRight: 4, verticalAlign: "middle" }} />
                      ...
                      {compRows.map((row, ri) => (
                        <tr key={ri} className={ri === 0 ? "bg-amber-50/50 text-slate-900" : "text-slate-700 border-t border-slate-100"}>
                          ...
                          {liveData.series.map((s) => {
                            ...
                            const pct = fmtPctAl(latestVal ?? null, val)
                            return (
      ...
      <Card className="overflow-hidden border-slate-200/80 shadow-sm">
        <CardHeader className="gap-3 md:flex-row md:items-end md:justify-between">
          <div className="space-y-1">
            <CardTitle className="text-xl text-[#02315a]">Aluminium coil</CardTitle>
            <CardDescription>
              Detailed aluminum ingot price trend. The dashboard shows only a compact recent preview.
            </CardDescription>
          </div>
        ...
        <CardContent className="space-y-4 p-3 pt-0 sm:p-6 sm:pt-0">
          ...
          <div className="flex flex-col gap-2 text-xs text-muted-foreground md:flex-row md:items-center md:justify-between">
            ...
            <a
              href={liveData?.spreadsheetUrl || "https://docs.google.com/spreadsheets/d/1RQb5fBTipFZPslbG60vP46DJZ8ZD9D7a7_eaKw718nM/edit?gid=1629194981#gid=1629194981"}
              target="_blank"
